<!--本文件由FirstUI授权予湖北道邦科技有限公司（手机号： 1  85     8 183 3064，身份证尾号：AURG00）专用，请尊重知识产权，勿私下传播，违者追究法律责任。-->
<template>
	<view class="fui-preview__wrap" :class="{'fui-preview__unlined':!isBorder,'fui-preview__background':!background}"
		...
		<view class="fui-preview__hd" :style="{paddingLeft:padding+'rpx',paddingRight:padding+'rpx'}" v-if="pvd[label] || pvd[value]">
			<view class="fui-preview__item">
				<text class="fui-preview__label" :style="{color:'#333',fontSize:hdLabelSize+'rpx'}">{{pvd[label] || ''}}</text>
				<text class="fui-preview__value-hd" :class="{'fui-preview__color':!hdValColor}" :style="{color:hdValColor,fontSize:hdValSize+'rpx'}">{{pvd[value] || ''}}</text>
			</view>
			<view :style="{background:borderColor,left:hdLeft+'rpx',right:hdRight+'rpx'}" class="fui-preview__border" :class="{'fui-preview__border-color':!borderColor}"></view>
		</view>
		<view class="fui-preview__bd" v-if="pvd[list] && pvd[list].length>0" :style="{paddingLeft:padding+'rpx',paddingRight:padding+'rpx'}">
			<view class="fui-preview__item" v-for="(item,index) in pvd[list]" :key="index">
				<text class="fui-preview__label" :class="[!labelColor && !item[lColor]?'fui-preview__gray':'','fui-preview__label-'+labelAlign]"
		...
		<view class="fui-preview__ft" :style="{'border-top-color':borderColor}" v-if="pvd[buttons] && pvd[buttons].length>0">
			<view :style="{'border-right-color':idx!==pvd[buttons].length-1?borderColor : 'transparent'}"
				...
				:hover-class="highlight?'fui-preview_hover':''" :hover-stay-time="150" v-for="(btn,idx) in pvd[buttons]" :key="idx" @tap="handleClick(idx)">
				<text :style="{fontSize:btnSize+'rpx',color:btn[color] || '#1087DD'}" :class="{'fui-preview__link':!btn[color]}">{{btn[text]}}</text>
			</view>
		</view>
	</view>
</template>
...
<style scoped>
	...
	.fui-preview__link {
		...
		/* #endif */
		/* #ifndef APP-NVUE */
		color: var(--fui-color-link, #1087DD) !important;
		/* #endif */
	}
