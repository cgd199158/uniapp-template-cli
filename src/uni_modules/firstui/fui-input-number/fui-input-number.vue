<!--本文件由FirstUI授权予湖北道邦科技有限公司（手机号： 1858    18  3  3 064，身份证尾号：AURG00）专用，请尊重知识产权，勿私下传播，违者追究法律责任。-->
<template>
	<view class="fui-input__number">
		...
		<input :type="type" v-model="inputValue" :disabled="disabled" @blur="blur" @input="onInput" class="fui-number__input"
			:style="{ border: 'none', color: color, fontSize: size + 'rpx', backgroundColor: backgroundColor, height: height + 'rpx', minHeight: height + 'rpx', width: width + 'rpx',borderRadius:radius+'rpx',marginLeft:margin+'rpx',marginRight:margin+'rpx' }" />
...
<script>
  import _ from 'lodash'
	export default {
		...
		methods: {
      // input事件在初始赋值时不会触发 陈国栋 20230714
