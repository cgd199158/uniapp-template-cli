// 本文件由FirstUI授权予湖北道邦科技有限公司（手机号：   185 8 1 833 0   64，身份证尾号：AURG00）专用，请尊重知识产权，勿私下传播，违者追究法律责任。
/**
 * 表单验证
 * https://doc.firstui.cn/
 **/

const getValueBySafe = (data, path, defaultValue) => {
  /**
   * 安全读取数据逻辑
...
const form = {
  ...
  validator: function (formData, rules, checkAll = false) {
    ...
    for (let item of rules) {
      ...
      //   (!~rule.indexOf("required") && getValueBySafe(formData, key).toString().length === 0)
      // ) {
      //   continue;
      // }
      ...
      if (ruleLen) {
        for (let i = 0; i < ruleLen; i++) {
          ...
          let isError = false;
		      const formValue = getValueBySafe(formData, key, '')
          switch (ruleItem) {
            case "required":
              isError = form._isNullOrEmpty(formValue);
              break;
            case "isMobile":
              isError = !form._isMobile(formValue);
              break;
            case "isEmail":
              isError = !form._isEmail(formValue);
              break;
            case "isCarNo":
              isError = !form._isCarNo(formValue);
              break;
            case "isIdCard":
              isError = !form._isIdCard(formValue);
              break;
            case "isAmount":
              isError = !form._isAmount(formValue);
              break;
            case "isNumber":
              isError = !form._isNumber(formValue);
              break;
            case "isChinese":
              isError = !form._isChinese(formValue);
              break;
            case "isNotChinese":
              isError = !form._isNotChinese(formValue);
              break;
            case "isEnglish":
              isError = !form._isEnglish(formValue);
              break;
            case "isEnAndNo":
              isError = !form._isEnAndNo(formValue);
              break;
            case "isEnOrNo":
              isError = !form._isEnOrNo(formValue);
              break;
            case "isSpecial":
              isError = form._isSpecial(formValue);
              break;
            case "isEmoji":
              isError = form._isEmoji(formValue);
              break;
            case "isDate":
              isError = !form._isDate(formValue);
              break;
            case "isUrl":
              isError = !form._isUrl(formValue);
              break;
            case "isSame":
              isError = !form._isSame(formValue, formData[value]);
              break;
            case "range":
              let range = null;
              try {
              ...
              isError = !form._isRange(getValueBySafe(formData, key, ''), range[0], range[1]);
              break;
            case "minLength":
              isError = !form._minLength(getValueBySafe(formData, key, ''), value);
              break;
            case "maxLength":
              isError = !form._maxLength(getValueBySafe(formData, key, ''), value);
              break;
            case "isKeyword":
              isError = !form._isKeyword(getValueBySafe(formData, key, ''), value);
              break;
            default:
              break;
          }
      ...
      if (validatorLen) {
        for (let model of validator) {
          ...
          if (func && !func(getValueBySafe(formData, key, ''), formData)) {
            result.isPassed = false;
            if (checkAll) {
